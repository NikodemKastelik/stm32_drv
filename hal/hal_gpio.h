...
__STATIC_INLINE void hal_gpio_mode_cfg(GPIO_TypeDef * const p_port, uint32_t pin, hal_gpio_mode_t mode);

__STATIC_INLINE void hal_gpio_af_cfg(GPIO_TypeDef * const p_port, uint32_t pin, hal_gpio_af_t af);

__STATIC_INLINE void hal_gpio_out_set(GPIO_TypeDef * const  p_port, uint32_t pin);

__STATIC_INLINE void hal_gpio_out_clr(GPIO_TypeDef * const p_port, uint32_t pin);

__STATIC_INLINE void hal_gpio_outtype_cfg(GPIO_TypeDef * const p_port, uint32_t pin, hal_gpio_outtype_t outtype);

__STATIC_INLINE void hal_gpio_outspd_cfg(GPIO_TypeDef * const p_port, uint32_t pin, hal_gpio_outspd_t outspd);

__STATIC_INLINE void hal_gpio_pull_cfg(GPIO_TypeDef * const p_port, uint32_t pin, hal_gpio_pull_t pull);

__STATIC_INLINE bool hal_gpio_input_check(const GPIO_TypeDef * const p_port, uint32_t pin);

__STATIC_INLINE void hal_gpio_cfg_lock(GPIO_TypeDef * const p_port, uint32_t pin_msk);
...
__STATIC_INLINE void hal_gpio_cfg_lock(GPIO_TypeDef * const p_port, uint32_t pin_msk)
{
    ...
    p_port->LCKR = pin_msk;
    p_port->LCKR = pin_msk | GPIO_LCKR_LCKK;
    uint32_t dummy_read = p_port->LCKR;
    (void)dummy_read;
}
